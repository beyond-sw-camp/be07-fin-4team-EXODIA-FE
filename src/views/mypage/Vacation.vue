<template>
    ...
    methods: {
      async fetchUserProfile() {
        try {
          ...
          if (this.userProfile.joinDate) {
            ...
            this.workDays = currentDate.diff(joinDate, 'days');
          }
      ...
      async fetchTodayAttendance() {
        try {
          ...
          this.attendanceData.clockInTime = data.inTime ? moment(data.inTime).format('HH:mm:ss') : 'N/A';
          this.attendanceData.clockOutTime = data.outTime ? moment(data.outTime).format('HH:mm:ss') : 'N/A';
          this.attendanceData.weeklyWorkHours = data.hoursWorked ? data.hoursWorked.toFixed(1) + '(시간)' : 'N/A';
  ...
  .custom-leave-table td, .custom-leave-table th {
    padding: 15px 30px;
    border: 1px solid #f5f5f5;
    font-weight: 700;
  }
