    ...
    <v-row justify="center" style="margin:0; text-align:center;">
        <v-col cols="6" sm="6">
            <v-text-field v-model="searchQuery" variant="underlined" placeholder="검색어를 입력하세요" append-icon="mdi-magnify"
                @click:append=searchFilter(searchQuery) style="margin-bottom: 20px;"></v-text-field>
        </v-col>
    </v-row>
        ...
        <v-row justify="center" style="margin:0; text-align:center; ">
            <v-col cols="12">
                ...
                <v-row v-for="(submit, index) in filteredSubmitList" :key="submit.id" oulined
                    ...
                    <v-col cols="3">{{ formatDate(submit.submitTime) }} {{ formatLocalTime(submit.submitTime) }}</v-col>
